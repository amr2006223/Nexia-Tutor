...
import { FiFileText, FiPlay, FiPlus, FiSearch, FiTrash, FiTrash2 } from "react-icons/fi";
...
export default function LearningPage(): JSX.Element {
  ...
  // Function to handle file upload for a specific index
  const handleKeyowrdExtraction = async (index: number): Promise<void> => {
    ...
    formData.append("file", fileToUpload); // Append file data with name
    ...
    try {
      ...
      lessonState.setLessonDetails(fileToUpload.name, response.data.keywords);
      // Handle response
          ...
          {uploadedFiles.map((file, index) => (
            ...
            >
              ...
              <Link
                ...
                onClick={() => handleKeyowrdExtraction(index)} href={"/TodayLesson"}              >
                Continue
              </Link>
